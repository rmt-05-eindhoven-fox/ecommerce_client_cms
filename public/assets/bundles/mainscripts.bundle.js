...
($.AdminAero.leftSideBar = {
  activate: function () {
    ...
    $(window).on('click', function (e) {
      var a = $(e.target)
      e.target.nodeName.toLowerCase() === 'i' && (a = $(e.target).parent()), !a.hasClass('bars') && r.isOpen() && a.parents('#leftsidebar').length === 0 && (a.hasClass('js-right-sidebar') || i.fadeOut(), t.removeClass('overlay-open'))
    }),
  ...
  checkStatuForResize: function (e) {
    var a = $('body')
    var r = $('.navbar .navbar-header .bars')
    var t = a.width()
    e &&
        a
    ...
    t < 1170 ? (t > 767 && a.addClass('ls-toggle-menu'), a.addClass('ls-closed'), r.fadeIn()) : (a.removeClass('ls-closed ls-toggle-menu'), r.fadeOut())
  },
  isOpen: function () {
...
($.AdminAero.rightSideBar = {
  activate: function () {
    ...
    $(window).on('click', function (e) {
      var a = $(e.target)
      e.target.nodeName.toLowerCase() === 'i' && (a = $(e.target).parent()), !a.hasClass('js-right-sidebar') && r.isOpen() && a.parents('#rightsidebar').length === 0 && (a.hasClass('bars') || i.fadeOut(), t.removeClass('open'))
    }),
...
function skinChanger () {
  $('.right-sidebar .choose-skin li').on('click', function () {
    var e = $('body')
    var a = $(this)
    var r = $('.right-sidebar .choose-skin li.active').data('theme')
...
function CustomScrollbar () {
  $('.sidebar .menu .list').slimscroll({ height: 'calc(100vh - 65px)', color: '#eeeeee', position: 'right', size: '1px', alwaysVisible: !1, borderRadius: '3px', railBorderRadius: '0' }),
  $('.navbar-right .dropdown-menu .body').slimscroll({ height: '330px', color: 'rgba(0,0,0,0.2)', size: '3px', alwaysVisible: !1, borderRadius: '3px', railBorderRadius: '0' }),
  $('.chat-widget').slimscroll({ height: '310px', color: 'rgba(0,0,0,0.4)', size: '2px', alwaysVisible: !1, borderRadius: '3px', railBorderRadius: '2px' }),
  $('.right-sidebar .slim_scroll').slimscroll({ height: 'calc(100vh - 70px)', color: 'rgba(0,0,0,0.4)', size: '2px', alwaysVisible: !1, borderRadius: '3px', railBorderRadius: '0' })
}
function CustomJs () {
  $('.light_dark input').on('change', function () {
    $(this).val() == 'dark' ? $('body').addClass('theme-dark') : $('body').removeClass('theme-dark')
  }),
...
$(function () {
  ...
  $('#search, #search #close').on('click keyup', function (e) {
    (e.target != this && e.target.id != 'close' && e.keyCode != 27) || $(this).removeClass('open')
  })
}),
$(function () {
  location.hash == '#dark' &&
